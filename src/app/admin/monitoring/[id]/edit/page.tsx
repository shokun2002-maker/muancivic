...
export default function EditMonitoringPage({ params }: Props) {
  const { id } = use(params);
  const router = useRouter();

  const [post, setPost] = useState<MonitoringDbPost | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      try {
        const data = await getAdminMonitoringPostById(id);
        if (!data) {
          alert("존재하지 않거나 삭제된 모니터링 리포트입니다.");
  ...
  const handleSubmit = async (
    ...
    await updateMonitoringPost(id, payload);
    alert("모니터링 리포트가 성공적으로 수정되었습니다.");
    router.push("/admin/monitoring");
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">모니터링 리포트 수정</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            등록된 모니터링 리포트 정보 및 상태를 수정합니다.
          </p>
        </div>
      </div>
