...
import { getAdminResources, deleteResource } from "@/lib/admin/resources";
import { ResourceDbRow } from "@/types/resource";
import { Plus, Edit2, Trash2, Search, Tag, FileText, ExternalLink } from "lucide-react";

export default function AdminResourcesPage() {
  const [resources, setResources] = useState<ResourceDbRow[]>([]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900">정책자료실 관리</h1>
          <p className="text-sm text-gray-500 mt-1">
            시민연대의 정책보고서, 포럼발제집, 조사분석 자료를 등록 및 관리합니다.
          </p>
        </div>
        <Link
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            <option value="토론·포럼">토론·포럼</option>
            <option value="정책질의">정책질의</option>
            <option value="조사·분석">조사·분석</option>
            <option value="공공자료">공공자료</option>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center text-gray-500">
          ...
          <p className="font-semibold text-gray-700">등록된 정책자료가 없습니다.</p>
          <p className="text-xs text-gray-400 mt-1">상단의 버튼을 눌러 새 자료를 등록해보세요.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50/80 border-b border-gray-200 text-gray-600 font-semibold">
                <tr>
                  ...
                  <th className="px-4 py-3.5">출처</th>
                ...
                {resources.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/60 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      {item.summary && (
                        <div className="text-xs text-gray-500 line-clamp-1 mt-1">{item.summary}</div>
                      )}
                      {item.file_url && (
                        <div className="inline-flex items-center gap-1 text-[11px] text-[#176B52] font-semibold mt-1">
